	...
	void _write_params_loc(std::string& str, ConfigLocation& loc) {
		...
		if ((pos = str.find(" ")) != std::string::npos && (delete_pos = str.find(";")) != std::string::npos) {
			...
			if (first_str == "autoindex") {
				...
				loc.getBodySize() = std::atoi(second_str.c_str());
			}
...
public:
	ConfigServers() {}
	ConfigServers(const ConfigServers &copy) {*this = copy;}
	...
	std::vector<ConfigServer>	&getConfigsServer() {
		return this->_serv;
	}
	~ConfigServers() {};
	ConfigServers	&operator=(const ConfigServers &copy) {
